...
class Hueco:
    def __init__(self, nombre, fecha_venta, grupo_compra, atributo, familias=None, familias_nombres=None):
        self.nombre = nombre
        self.fecha_venta = fecha_venta
        self.grupo_compra = grupo_compra
        ...
        # familias_nombres: lista con los nombres de las familias
        self.familias_nombres = familias_nombres or []
...
urns_familias = cfg.get("urns_familias", DEFAULT_URNS.get("urn_familias", {}))
...
def crear_hueco_api(hueco, token, base_url, collection_id, campaign_urn):
    ...
    # Añadir collectionId solo si existe (si no es None)
    if collection_id is not None:
        payload["productPlaceholder"]["collectionId"] = collection_id
    ...
    if response.status_code == 201:
        try:
            ...
            # Solo asociar a colección si collection_id existe
            if collection_id is not None:
            ...
            with open(ids_log_file, "a", encoding="utf-8") as f:
                if collection_id is not None:
                    ...
                    f.write(f"- HUECO (SIN COLECCIÓN): {hueco.nombre} | ID: {hueco_id}\n")
        except Exception:
...
colecciones_dict = {}
# Lista para almacenar huecos sin colección
huecos_sin_coleccion = []
...
for index, row in df.iterrows():
    ...
    # Evitar crear objetos si la fecha o descripción es NaT o vacía
    if pd.notna(fecha_venta) and pd.notna(nombre):
        ...
        if pd.notna(familias_cell) and str(familias_cell).strip():
            familias_nombres = [f.strip() for f in str(familias_cell).split(',') if f.strip()]
            familias_urns = [urns_familias.get(name) for name in familias_nombres]
        ...
        # Si hay colección, crear o actualizar la colección
        if pd.notna(nombre_coleccion) and str(nombre_coleccion).strip():
            ...
            hueco = Hueco(nombre, fecha_venta, grupo_compra, atributo, familias=familias_urns, familias_nombres=familias_nombres)
            coleccion.add_hueco(hueco)
        else:
            # Si no hay colección, añadir el hueco a la lista de huecos sin colección
            hueco = Hueco(nombre, fecha_venta, grupo_compra, atributo, familias=familias_urns, familias_nombres=familias_nombres)
            huecos_sin_coleccion.append(hueco)
    ...
    for coleccion in colecciones_dict.values():
        ...
        for hueco in coleccion.huecos:
            file.write(
                ...
                f"Atributo: {hueco.atributo}, Familias: {', '.join(hueco.familias_nombres) if hueco.familias_nombres else 'N/A'}\n"
            )
        file.write("\n")
    
    # Escribir huecos sin colección
    if huecos_sin_coleccion:
        file.write("HUECOS SIN COLECCIÓN:\n\n")
        for hueco in huecos_sin_coleccion:
            file.write(
                f"  - Hueco: {hueco.nombre}, Fecha Venta: {hueco.fecha_venta}, "
                f"Grupo Compra: {hueco.grupo_compra},  "
                f"Atributo: {hueco.atributo}, Familias: {', '.join(hueco.familias_nombres) if hueco.familias_nombres else 'N/A'}\n"
...
if confirmacion.strip().lower() == 'sí':
    ...
    total_huecos = sum(len(coleccion.huecos) for coleccion in colecciones_dict.values()) + len(huecos_sin_coleccion)
    if token:
        ...
        # Crear colecciones y sus huecos
        for coleccion in colecciones_dict.values():
            # Crear colecciones
        ...
        # Crear huecos sin colección
        for hueco in huecos_sin_coleccion:
            crear_hueco_api(hueco, token, base_url, None, campaign_urn)
            print(f"Colecciones creadas {nColecciones} de {total_colecciones}. Huecos creados: {nHuecos} de {total_huecos}", end='\r', flush=True)
